...
public class User_Routes_History_DB_Helper extends SQLiteOpenHelper {
    ...
    private final String TABLE_COLUMN_DAY_COUNT = "dayCount";
    ...
    public void onCreate(SQLiteDatabase db) {
        String sqlQuery = String.format("CREATE TABLE IF NOT EXISTS %s (" +
                        " %s text, %s text, %s text, %s text, %s text, %s INTEGER, %s text PRIMARY KEY)", TABLE_NAME, TABLE_COLUMN_TRAIN_NUMBER,
                TABLE_COLUMN_TRAIN_NAME, TABLE_COLUMN_SOURCE_STATION_CODE, TABLE_COLUMN_DESTINATION_STATION_CODE,
                TABLE_COLUMN_ADDED_ON, TABLE_COLUMN_DAY_COUNT, TABLE_COLUMN_UNIQUE_ID);
        db.execSQL(sqlQuery);
    ...
    public void addHistoryInDB(String trainNumber, String trainName, String source, String destination, int dayCount) {
        ...
        values.put(TABLE_COLUMN_DAY_COUNT, dayCount);
    ...
    public ArrayList<User_History_Structure> getHistory() {
        ...
        while (cursor.moveToNext()) {
            ...
            int dayCount = cursor.getInt(cursor.getColumnIndexOrThrow(TABLE_COLUMN_DAY_COUNT));
            history.add(new User_History_Structure(trainNumber, trainName, sourceCode, destinationCode, dayCount));
        }
        db.close();
        cursor.close();
        return history;
