...
public class ShowTrainInfoDialog {
    ...
    public static void showExtraTrainInfo(String trNum, Train_Schedule_Structure trainData, Context context) {
        new Thread(() -> {
            try (Train_Schedule_DB_Helper dbHelper = new Train_Schedule_DB_Helper(context)) {
                Train_Schedule_Structure myTrainData;
                if (trNum != null) {
                    myTrainData = dbHelper.getTrainDataByTrainNumber(trNum);
                } else {
                ...
                String finalFormattedJourneyTime = formattedJourneyTime;
                ((Activity) context).runOnUiThread(() -> {
                    ...
                    infoTrainJourneyTime.setText(finalFormattedJourneyTime);
                    infoTrainStops.setText(String.valueOf(totalStops));
                    infoTrainTotalDistance.setText(destinationStation.getDistance() + " Km");
